...
**선택지의 비용을 여기서 정직하게 보여준다.** partial clone과 shallow는
처음 기다리는 시간을 크게 줄이는 대신, **그 기다림을 나중으로 미룬다** —
과거 파일을 열 때마다 서버를 다시 기다리고 오프라인에서는 아예 막힌다.

목적 함수가 대기 시간이므로(ADR-56) 판단 기준은 "총량이 느는가"가 아니라
**"예측 가능한 한 번의 대기와 예측 불가능한 반복 정지 중 무엇이 나은가"**다.
후자가 나쁘다고 보아 기본값은 전체 복제이고(ADR-6), 다른 선택을 하면
**무엇을 잃는지 함께 적는다.**
