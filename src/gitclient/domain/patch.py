...
class FilePatch:
    ...
    def fingerprint(self) -> str:
        ...
        부분 스테이징은 사용자가 **화면에서 본** 패치의 (헝크, 줄) 좌표를
        넘기는데, 엔진은 적용 시점에 워킹 트리를 다시 읽는다. 그 사이 파일이
        바뀌면 같은 좌표가 다른 줄을 가리켜 **사용자가 고르지 않은 내용이
        조용히 스테이징된다** — 오류도 경고도 없이.

        화면이 본 패치와 적용할 패치가 같은지 확인하는 데 쓴다.
        (`fast_forward`/`merge`의 `expected_branch`와 같은 취지)

        구분자 대신 길이를 함께 넣는다 — 구분자를 쓰면 그 문자가 내용에
        들어 있을 때 서로 다른 패치가 같은 값을 낼 수 있다.
        """
